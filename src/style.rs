...
pub enum SegmentedButtonStyles {
    #[default]
    Default,
    Custom(Box<dyn StyleSheet<Style = Theme>>),
}

impl SegmentedButtonStyles {
...
impl StyleSheet for Theme {
    type Style = SegmentedButtonStyles;

    fn active(&self, style: &Self::Style) -> Appearance {
        match style {
            SegmentedButtonStyles::Default => Appearance::default(),
            SegmentedButtonStyles::Custom(custom) => custom.active(self),
        }
    }

    fn hovered(&self, style: &Self::Style) -> Appearance {
        if let SegmentedButtonStyles::Custom(custom) = style {
